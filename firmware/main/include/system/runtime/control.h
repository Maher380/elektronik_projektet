...
public:
    ...
    /**
     * @brief Evaluate local safety conditions and calculate motor output.
     * @param pathSensor Sensor for the path selected by autonomous navigation.
     * @param requestedDuty Navigation duty, additionally capped by runtime configuration.
     * @return Normalized forward motor command in range 0.0 to 0.5.
     */
    float evaluate(const std::array<float, IrSensorCount>& distancesCm,
                   std::uint32_t nowMs, std::size_t pathSensor = 1U,
                   float requestedDuty = 0.5F) noexcept;
